...
const PendingOrdersPage = () => {
  ...
  const handleAcceptOrder = async (orderId) => {
    ...
    try {
      // In a real app, you would call your API to accept the order
    ...
    <div className="px-4 py-6 sm:px-0">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600">Orders available for delivery</p>
        </div>
      </div>
              ...
              <div key={order.id} className="bg-white rounded-xl shadow-sm overflow-hidden">
                <div className="p-4 border-b border-gray-100">
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="font-medium text-gray-900">{order.restaurantName || "Restaurant"}</h3>
                      <div className="flex items-center text-sm text-gray-500 mt-1">
                        <FiMapPin size={12} className="mr-1" />
                        <span>{order.restaurantAddress || "Address unavailable"}</span>
                      </div>
                    </div>
                ...
                <div className="p-4">
                  <div className="flex items-center text-sm text-gray-600 mb-3">
                    ...
                    <span className="ml-1">{order.customerName || "Customer"}</span>
                  </div>
                  
                  <div className="flex items-start text-sm text-gray-600 mb-3">
                    <FiMapPin size={14} className="mr-2 mt-1" />
                    <span>{order.deliveryAddress || "Address unavailable"}</span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    ...
                    <div className="flex flex-col items-center justify-center p-2 bg-gray-50 rounded-md">
                      <div className="flex items-center text-gray-700">
                        ...
                        <span className="font-medium">{order.orderTime ? formatTimeAgo(order.orderTime) : "N/A"}</span>
                      </div>
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                    ...
                    <div className="mt-4">
                      <div className="flex justify-between items-center mb-4">
                        <div>
                          <h4 className="font-medium text-gray-900">{selectedOrder.restaurantName || "Restaurant"}</h4>
                          <p className="text-sm text-gray-500">{selectedOrder.restaurantAddress || "Address unavailable"}</p>
                        </div>
                        <span className="text-sm font-medium text-gray-500">#{selectedOrder.orderId || selectedOrder.id}</span>
                      </div>
                      ...
                      <div className="border-t border-b border-gray-200 py-4 mb-4">
                        ...
                        <ul className="space-y-2">
                          {selectedOrder.items && selectedOrder.items.length > 0 ? (
                            ...
                            <li className="text-sm text-gray-500">No items available</li>
                          )}
                        </ul>
                      </div>
                      ...
                      <div className="mb-4">
                        ...
                        <div className="space-y-2 text-sm">
                          <div className="flex items-start">
                            ...
                            <div>
                              <p className="text-gray-700">{selectedOrder.customerName || "Customer"}</p>
                            </div>
                          </div>
                          <div className="flex items-start">
                            <FiMapPin size={14} className="mr-2 mt-1" />
                            <div>
                              <p className="text-gray-700">{selectedOrder.deliveryAddress || "Address unavailable"}</p>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-gray-50 p-4 rounded-md">
                        <div className="flex justify-between mb-2">
                          ...
                          <span className="font-medium">{formatCurrency(selectedOrder.total || 0)}</span>
                        </div>
                        <div className="flex justify-between mb-2">
                          <span className="text-sm text-gray-600">Order Time:</span>
                          <span className="font-medium">{selectedOrder.orderTime ? new Date(selectedOrder.orderTime).toLocaleString() : "N/A"}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-sm text-gray-600">Status:</span>
                          <span className="font-medium text-yellow-600">Available</span>
                        </div>
                      </div>
                    </div>
                  </div>
